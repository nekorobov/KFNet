...
tf.app.flags.DEFINE_integer('max_steps', 400000, """Max training iteration.""")
tf.app.flags.DEFINE_integer('display', 10, """Interval of loginfo display.""")
tf.app.flags.DEFINE_integer('stepvalue', 80000, """Step interval to decay learning rate.""")
tf.app.flags.DEFINE_integer('snapshot', 20000, """Step interval to save the model.""")
...
def get_training_data(image_list, label_list, spec, is_training=True):
    ...
    with tf.name_scope('data_queue'):
        with tf.device('/CPU:0'):
            index_queue = tf.train.string_input_producer(
                indexes, shuffle=(is_training or FLAGS.shuffle))
            ...
            for i in range(2):
                ...
                image = tf.squeeze(
                    tf.image.crop_and_resize(
                        [image], [[0, 0, 1, spec.image_size[1] / (spec.image_size[1] + 12.)]], [0],
                        (spec.image_size[0], spec.image_size[1])))
                ...
                label_shape = [spec.image_size[0] // 8, spec.image_size[1] // 8, 4]
                label = tf.reshape(label, label_shape)
                label = tf.squeeze(
                    tf.image.resize_nearest_neighbor([label],
                                                     [spec.image_size[0], spec.image_size[1]]))
...
def run(image_list, label_list, spec, is_training=True):
    with tf.name_scope('data'):
        images, gt_coords, masks, group_indexes = get_training_data(image_list, label_list, spec,
                                                                    is_training)

    with tf.device('/device:CPU:0'):
        gt_coords = tf.image.resize_nearest_neighbor(
            gt_coords, [spec.crop_size[0] / 8, spec.crop_size[1] / 8])
        masks = tf.image.resize_nearest_neighbor(masks,
                                                 [spec.crop_size[0] / 8, spec.crop_size[1] / 8])
    ...
    kfnet = KFNet(
        ...
        spec.u,
        spec.v,
        train_scoordnet=False,
        train_oflownet=is_training,
        reuse=tf.AUTO_REUSE)

    with tf.name_scope('loss'):
        measure_coord_loss, measure_coord_accuracy = kfnet.CoordLossWithUncertainty(
            noise_gt_coords, gt_uncertainty, gt_coords, masks)
        temp_coord_loss, temp_coord_accuracy = kfnet.TemporalCoordLoss(gt_coords, shift_masks)

        images = tf.image.resize_bilinear(images, [spec.crop_size[0] // 8, spec.crop_size[1] // 8])
    ...
    with tf.Session(config=config) as sess:
        ...
        while step <= FLAGS.max_steps:
            ...
            if step % FLAGS.display == 0 or not FLAGS.is_training:
                ...
                print(format_str %
                      (datetime.now(), epoch, step, out_group_indexes[0], out_group_indexes[1],
                       out_loss, out_mea_loss, out_mea_accuracy,
                       out_temp_loss, out_temp_accuracy, out_image_loss, out_smooth_loss,
...
def main(_):
    ...
    train(image_list, label_list, FLAGS.model_folder, snapshot, step, FLAGS.debug, FLAGS.gpu)
